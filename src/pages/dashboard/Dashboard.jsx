    ...
    <div className="page-body">
      ...
      {usuario?.papel === 'produtor' && (
        <>
          <div className="stat-cards">
            <div className="stat-card">
              ...
              <div className="stat-info">
                <h3>{mockColheitas.filter((c) => c.produtorId === usuario.id).length}</h3>
            ...
            <div className="stat-card">
              <div className="stat-icon">⭐</div>
              <div className="stat-info">
                <h3>{usuario.reputacao ?? '—'}</h3>
                <p>Reputação ({usuario.totalAvaliacoes} avaliações)</p>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            ...
            <div className="card">
              <div className="card-header">
                <span className="card-title">🔔 Transações Pendentes</span>
              </div>
              {pendentes.length === 0 ? (
        ...
        <>
          <div className="stat-cards">
            ...
            <div className="stat-card">
              <div className="stat-icon">🚢</div>
              <div className="stat-info">
                <h3>{mockRotas.filter((r) => r.freteiro.id === usuario.id).length}</h3>
      ...
      {usuario?.papel === 'admin' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
          ...
          <div className="card">
            <div className="card-header">
              <span className="card-title">📊 Resumo da Plataforma</span>
            </div>
            <div style={{ fontSize: '0.9rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <div className="flex-between"><span className="text-muted">Rotas ativas</span><span className="font-bold">{mockRotas.filter((r) => r.disponivel).length}</span></div>
            </div>
          </div>
        </div>
      )}
